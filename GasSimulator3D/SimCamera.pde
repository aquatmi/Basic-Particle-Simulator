...
class SimCamera{
  ...
  void setHUDArea(int left, int top, int right, int bottom){
    hudArea = new Rect(left,top,right,bottom);
  }
  ...
  void strafe(int dir){
    ...
    if(dir == UP){
      cameraPos = PVector.add(cameraPos, cameraUpVector);
      cameraLookat = PVector.add(cameraLookat, cameraUpVector);
    }
    if(dir == DOWN){
      cameraPos = PVector.sub(cameraPos, cameraUpVector);
      cameraLookat = PVector.sub(cameraLookat, cameraUpVector);
    }
