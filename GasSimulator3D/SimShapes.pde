...
abstract class SimTransform{
  ...
  void resetTransform() {
    setTransformAbs(vec(0, 0, 0), 1, 0, 0, 0);
  }
  ...
  boolean isRotated() {
    if ( this.rotateX == 0.0 && this.rotateY == 0.0 && this.rotateZ == 0.0) return false;
    return true;
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////
...
class SimBox extends SimTransform{
  ...
  public boolean isPointInside(PVector p) {
    if ( isRotated() == false ) {
      ...
      if (  (p.x < maxCornerTrans.x &&  p.x > minCornerTrans.x)   &&
        (p.y < maxCornerTrans.y &&  p.y > minCornerTrans.y)   &&
        (p.z < maxCornerTrans.z &&  p.z > minCornerTrans.z)   ) return true;
      return false;
    }
    // tbd, work out point inside for OBB
  ...
  public boolean intersectsBox(SimBox otherBox) {
    ...
    println("Boundin Box intersection not implemented yet");
    return false;
  }
